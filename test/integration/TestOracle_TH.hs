...
$(genSqlWith defGenOpts { goSel = ''SelOne } "or4" orW (\f -> [st|select 123.45, 123.45e, cast(123.45 as numeric(10,2)) as fldtest, cast (123.45 as number), cast (123.45 as decimal), cast (123.45 as float) from dual where #{f "1=1"} |]))

-- every literal is a Maybe in oracle: in sqlserver strings are not null but the rest are by default Maybe
$(genSqlWith defGenOpts { goSel = ''SelOne } "or5" orW (\f -> [st|select cast(123.45 as float(5)) as fldtest, cast (123.45 as real), 'abcdef' from dual where #{f "1=1"} |]))
