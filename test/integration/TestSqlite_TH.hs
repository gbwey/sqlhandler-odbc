...
type DatetimeN1 =
  '(Parsetime UTCTime "%Y-%m-%d %H:%M:%S" Id, 'True,
    Formattime "%Y-%m-%d %H:%M:%S", String)
...
type FromSeconds (t :: Type) = '(ShowP >> Parsetime t "%s" Id, 'True, Formattime "%s" >> ReadP Integer, Integer)
{-
